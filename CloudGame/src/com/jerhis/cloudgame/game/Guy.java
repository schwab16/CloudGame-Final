...
public class Guy {
	...
	public final float JUMPVELOCITY = 6, MOMENTUM = 1f, GRAVITY = 9,
			...
			TILTMOMENTUM = 8f, REGULAR_JUMP = 4.0f, SUPER_JUMP = 10.8f;
	...
	public void update(float delta, float accelX, float accelY, float accelZ, float height) {
		velY -= delta * GRAVITY;
		
		if (Math.abs(accelY) < 0.1)
			velX = 0;
		else velX = accelY * TILTMOMENTUM;
